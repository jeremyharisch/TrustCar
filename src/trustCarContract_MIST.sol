...
function seeGarage( address _adr) public  {
    garage_struct storage gar = garage_accounts[_adr];
...
function seeCarData( address _adr) public {
    //car_struct car = car_accounts[_adr];
     getCarData(car_accounts[_adr].name, car_accounts[_adr].yearOfConstruction,car_accounts[_adr].license_plate,car_accounts[_adr].owner,car_accounts[_adr].odometer,car_accounts[_adr].place);
...
function updateOdometer(address _adr, uint _odo)public onlyGarage{
    require(car_accounts[_adr].addr != 0);
...
function getHistory(address _adr) public   {
    ...
    for(i = 0; i < car_accounts[_adr].history.length; i++ ){
         showHistoryEntry( car_accounts[_adr].history[i].date, car_accounts[_adr].history[i].garage, car_accounts[_adr].history[i].content);
    }
